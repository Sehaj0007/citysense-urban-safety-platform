          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {user ? (
              <div className="flex items-center space-x-4">
                <Link to={user.role === 'admin' ? '/admin' : '/dashboard'} className="flex items-center hover:text-red-400 transition">
              ...
              <div className="flex items-center space-x-4">
                ...
                <Link to="/register" className="bg-red-600 hover:bg-red-700 px-4 py-2 rounded-md transition">
                  Sign Up
                </Link>
              </div>
            )}
      ...
      {isOpen && (
        <div className="md:hidden bg-slate-800">
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            <Link to="/report" className="block px-3 py-2 rounded-md hover:bg-slate-700">Report Incident</Link>
            {user ? (
              <>
                <Link to={user.role === 'admin' ? '/admin' : '/dashboard'} className="block px-3 py-2 rounded-md hover:bg-slate-700">Dashboard</Link>
                <button onClick={handleLogout} className="block w-full text-left px-3 py-2 rounded-md hover:bg-slate-700">Logout</button>
              </>
            ) : (
              <>
                <Link to="/login" className="block px-3 py-2 rounded-md hover:bg-slate-700">Login</Link>
                <Link to="/register" className="block px-3 py-2 rounded-md hover:bg-slate-700">Sign Up</Link>
              </>
            )}
          </div>
        </div>
